...
@implementation NSData (GeneralSmart)

//: - (NSData *)aes256DecryptWithKey:(NSString *)key vector:(NSString *)vector {
- (NSData *)effectuality:(NSString *)key pointFemale:(NSString *)vector {
...
void flexibleFlow(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)key:(NSString *)key {
    ...
    flexibleFlow(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
...
- (NSData *)resolve:(NSString *)key totalimately:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];
