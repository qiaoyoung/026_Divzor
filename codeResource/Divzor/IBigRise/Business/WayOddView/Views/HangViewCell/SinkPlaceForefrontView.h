...
@protocol StirDelegate <NSObject>
...
- (void)circleIndex:(SinkPlaceForefrontView *)menuView margin:(PortView *)menuCell submitBod:(NSInteger)index;

//: - (void)ffDropDownMenuViewWillAppear;
- (void)magnituderyBrand;
...
@interface SinkPlaceForefrontView : UIView
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage SmartWayBasedModel instances
...
@property (nonatomic, assign) CGFloat inputFloat;
...
@property (nonatomic, assign) UIColor *reasonObserve;
...
@property (nonatomic, assign) CGFloat changeProduce;
...
@property (nonatomic, assign) CGSize collect;
...
@property (nonatomic, assign) CGFloat totaleraction;
...
@property (nonatomic, assign) CGFloat letterFloat;
...
@property (nonatomic, assign) FFDropDownMenuViewAnimateType turn;
...
@property (nonatomic, assign) CGFloat boundTrigger;
...
@property (nonatomic, assign) CGFloat argument;
...
@property (nonatomic, weak) id<StirDelegate> commentMaxxed;
...
@property (nonatomic, assign) CGSize highlight;
...
@property (nonatomic, assign) CGFloat principalProduce;
...
@property (nonatomic, strong) UIColor *stick;
 ...
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage SmartWayBasedModel instances or the SmartWayBasedModel's subclass instances )
 */
//: @property (nonatomic, strong) NSArray *menuModelsArray;
@property (nonatomic, strong) NSArray *my;
...
@property (nonatomic, assign) CGFloat colorbegin;


/** 2、cell的类名, 必须是FFDropDownMenuBasedCell的子类
 *               若用框架自带的cell,直接传@"HangViewCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"HangViewCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of PortView
 *              If want to use this framework's drop-down nemu cell, direct pass @"HangViewCell"
...
///   >>步骤:
///   1、 [SinkPlaceForefrontView alloc] init]
///   2、 对需要赋值的属性进行赋值
...
@property (nonatomic, assign) UIColor *stateEnter;
...
@property (nonatomic, strong) UIColor *orientationLab;
