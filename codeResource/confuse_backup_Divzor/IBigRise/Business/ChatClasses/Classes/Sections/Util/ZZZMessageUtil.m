...
+ (NSString *)messageContent:(NIMMessage*)message {
    ...
    switch (message.messageType) {
        ...
        case NIMMessageTypeAudio:
            text = LangKey(@"Audio"); //@"[语音]".string_localized;
            break;
        case NIMMessageTypeImage:
            text = LangKey(@"Image");//@"[图片]".string_localized;
            break;
        case NIMMessageTypeVideo:
            text = LangKey(@"Video");//@"[视频]".string_localized;
            break;
        case NIMMessageTypeLocation:
            text = LangKey(@"Location");//@"[位置]".string_localized;
            break;
        case NIMMessageTypeNotification:{
            return [self notificationMessageContent:message];
        }
        case NIMMessageTypeFile:
            text = LangKey(@"File");//@"[文件]".string_localized;
            break;
...
+ (NSString *)notificationMessageContent:(NIMMessage *)message{
    ...
    if (object.notificationType == NIMNotificationTypeNetCall) {
        ...
        if (content.callType == NIMNetCallTypeAudio) {
            return LangKey(@"Internet_call");//@"[网络通话]".string_localized;
        }
        return LangKey(@"Video_chat");//@"[视频聊天]".string_localized;
    }
    if (object.notificationType == NIMNotificationTypeTeam) {
        NIMTeam *team = [[NIMSDK sharedSDK].teamManager teamById:message.session.sessionId];
        if (team.type == NIMTeamTypeNormal) {
            return LangKey(@"Group_chat_information_update");//@"[讨论组信息更新]".string_localized;
        }else{
            ...
            return LangKey(@"Group_information_update");//@"[群信息更新]".string_localized;
        }
    }
    
    if (object.notificationType == NIMNotificationTypeSuperTeam) {
        return LangKey(@"Super_Group_Information_Update");//@"[超大群信息更新]".string_localized;
    }
