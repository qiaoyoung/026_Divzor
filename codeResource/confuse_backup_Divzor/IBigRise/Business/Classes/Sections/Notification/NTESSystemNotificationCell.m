    ...
    if(hideActionButton) {
        ...
        switch (self.notification.handleStatus) {
            case NotificationHandleTypeOk:
                self.handleInfoLabel.text = LangKey(@"message_helper_already_ok");//@"已同意".string_localized;
                break;
            case NotificationHandleTypeNo:
                self.handleInfoLabel.text = LangKey(@"message_helper_already_no");//@"已拒绝".string_localized;
                break;
            case NotificationHandleTypeOutOfDate:
                self.handleInfoLabel.text = LangKey(@"message_helper_already_time");//@"已过期".string_localized;
                break;
            default:
...
//    [self.avatarImageView nim_setImageWithURL:url placeholderImage:sourceMember.avatarImage options:SDWebImageRetryFailed];
    ...
    switch (type) {
        ...
        {
            NSString *message_helper_apply_to_group = LangKey(@"message_helper_apply_to_group");//@"申请加入群".string_localized
        ...
        {
            NSString *contact_fragment_group = LangKey(@"contact_fragment_group");//@"群".string_localized
            NSString *message_helper_refuse_you = LangKey(@"message_helper_refuse_you");//@"拒绝你加入".string_localized
            NIMTeam *team = [[NIMSDK sharedSDK].teamManager teamById:self.notification.targetID];
        ...
        {
            NSString *contact_fragment_group = LangKey(@"contact_fragment_group");//@"群".string_localized
            NSString *invite_you_group = LangKey(@"invite_you_group");//@"邀请你加入".string_localized
            NIMTeam *team = [[NIMSDK sharedSDK].teamManager teamById:self.notification.targetID];
        ...
        case NIMSystemNotificationTypeTeamIviteReject:
        {
            ...
            NSString *message_helper_invite = LangKey(@"message_helper_invite");//@"邀请".string_localized
            NIMTeam *team = [[NIMSDK sharedSDK].teamManager teamById:self.notification.targetID];
            self.messageLabel.text = [NSString stringWithFormat:@"%@ %@ %@",
        ...
        {
            NSString *apply_join_supergroup = LangKey(@"apply_join_supergroup");//@"申请加入超大群 %@".string_localized
            ...
            if ([object isKindOfClass:[NIMUserAddAttachment class]]) {
                ...
                switch (operation) {
                    case NIMUserOperationAdd:
                        text = LangKey(@"message_helper_already_add");//@"已添加你为好友".string_localized;
                        break;
                    case NIMUserOperationRequest:
                        text = LangKey(@"request_add_you_as_a_friend");//@"请求添加你为好友".string_localized;
                        break;
                    case NIMUserOperationVerify:
                        text = LangKey(@"message_helper_allow_you_apply");//@"通过了你的好友请求".string_localized;
                        break;
                    case NIMUserOperationReject:
                        text = LangKey(@"message_helper_refuse_you_apply");//@"拒绝了你的好友请求".string_localized;
                        break;
                    default:
                        break;
                }
