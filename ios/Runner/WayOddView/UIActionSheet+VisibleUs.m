...
static char colorStickUtility;


//: @implementation UIActionSheet (NTESBlock)
@implementation UIActionSheet (VisibleUs)
...
- (void)weather
{
    ...
    objc_setAssociatedObject(self,&colorStickUtility,nil,OBJC_ASSOCIATION_COPY);
}

//: - (void)showInView: (UIView *)view completionHandler: (ActionSheetBlock)block
- (void)bringHome: (UIView *)view show: (ActionSheetBlock)block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&colorStickUtility,block,OBJC_ASSOCIATION_COPY);
    ...
    else
    {
        ...
        UITabBar *tabbar = [self limbRadio];
        //: if (tabbar)
        if (tabbar)
        {
        ...
        else
        {
            ...
            [self performSelector:@selector(bringHomeEntity:)
                       //: withObject:view
                       withObject:view
                       //: afterDelay:1];
                       afterDelay:1];
...

{
    ...
    ActionSheetBlock block = [objc_getAssociatedObject(self, &colorStickUtility) copy];
    //: objc_setAssociatedObject(self,&kUIActionSheetBlockAddress,nil,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&colorStickUtility,nil,OBJC_ASSOCIATION_COPY);
