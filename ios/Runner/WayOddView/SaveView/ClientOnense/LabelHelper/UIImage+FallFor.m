...
@implementation UIImage (FallFor)
...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(histories:));
...
+ (UIImage *)histories:(NSString *)name {
    ...
    UIImage *image = [[Reinstate shouldRaw] histories:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self histories:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
